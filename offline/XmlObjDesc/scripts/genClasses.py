...
#--------------------------------------------------------------------------------
  def genSmartRefFunctionalMethod(self, srs, scopeName=''):
    ...
    if not scopeName:
      ...
      s += '  /// Get event\n'
      s += '  EventObject* event(const std::string& eventName);\n\n'
      ...
      for sr in srs:
        ...
        s += '  }\n'
      s += '}\n\n'
      s += 'inline EventObject* ' + scopeName + '::event(const std::string& eventName)\n{\n'
      for sr in srs:
        s += '  if (eventName == "' + sr['attrs']['type'] + '") { \n'
...
#--------------------------------------------------------------------------------
  def genGetSetRelMethod(self,rel,what,scopeName=''):
    ...
    if what in ['gets', 'gets_c']:
      ret = self.tools.genReturnFromStrg('SmartRef&',self.generatedTypes,scopeName) + ' '
    if what == 'getse_c':
    ...
    if what == 'sett' and rel['multiplicity'] != '1': 
      ...
      param = self.tools.genParamFromStrg('SmartRef') + ' value'
    elif what == 'setse':
