...
int EDMManager::getPriorityWithPath(const std::string& path)
{
    EDMRegistration* reg = this->getRegWithPath(path);
    return reg ? reg->getPriority() : -1;
}

EDMRegistration::StringVector EDMManager::getEventNameWithPath(const std::string& path)
{
    EDMRegistration* reg = this->getRegWithPath(path);
    return reg ? reg->getEventName() : EDMRegistration::StringVector();
}

std::string EDMManager::getHeaderNameWithPath(const std::string& path)
{
    EDMRegistration* reg = this->getRegWithPath(path);
    return reg ? reg->getHeaderName() : "unknown";
}
...
EDMRegistration* EDMManager::getRegWithPath(const std::string& path)
{
    RegVector::iterator it, end = m_regs.end();
    for (it = m_regs.begin(); it != end; ++it) {
        if ((*it)->getPath() == path) {
    ...
    LogError << "Unknown data path: " << path
             << std::endl;
    return 0;
}
